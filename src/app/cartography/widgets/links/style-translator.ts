...
export class StyleTranslator {
  ...
  static normalizeBezierCurviness(bezierCurviness?: number): number {
    ...
    return Math.max(StyleTranslator.BEZIER_CURVINESS_MIN, Math.min(StyleTranslator.BEZIER_CURVINESS_MAX, steppedValue));
  }
  ...
  static getContinuousOrientation(source: [number, number], target: [number, number]): ConnectorOrientation {
  ...
  private static getStateMachinePath(
    ...
    const controlPoint: [number, number] = [midpointX + tangentX * curveOffset, midpointY + tangentY * curveOffset];
  ...
  private static getJsPlumbBezierPath(
    ...
    const { sourceControlPoint, targetControlPoint } = StyleTranslator.getBezierControlPoints(
      source,
      target,
      majorAnchor,
      sourceOrientation,
  ...
  private static sanitizeOrientation(orientation: ConnectorOrientation): ConnectorOrientation {
    ...
    return [clampToOrientationValue(orientation[0]), clampToOrientationValue(orientation[1])];
  }
  ...
  private static findBezierControlPoint(
    ...
    if (!isPerpendicular) {
      if (sourceOrientation[0] === 0) {
        controlPoint[0] =
          sourceAnchorPosition[0] < targetAnchorPosition[0] ? point[0] + minorAnchor : point[0] - minorAnchor;
      } else {
      ...
      if (sourceOrientation[1] === 0) {
        controlPoint[1] =
          sourceAnchorPosition[1] < targetAnchorPosition[1] ? point[1] + minorAnchor : point[1] - minorAnchor;
      } else {
      ...
      if (targetOrientation[0] === 0) {
        controlPoint[0] =
          targetAnchorPosition[0] < sourceAnchorPosition[0] ? point[0] + minorAnchor : point[0] - minorAnchor;
      } else {
      ...
      if (targetOrientation[1] === 0) {
        controlPoint[1] =
          targetAnchorPosition[1] < sourceAnchorPosition[1] ? point[1] + minorAnchor : point[1] - minorAnchor;
      } else {
