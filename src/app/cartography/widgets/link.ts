...
export class LinkWidget implements Widget {
  ...
  public draw(view: SVGSelection) {
    ...
    link_body
      .filter(l => l.capturing && !l.suspend && !(l.filters.bpf || l.filters.corrupt || l.filters.delay || l.filters.frequency_drop || l.filters.packet_loss))
      .append<SVGGElement>('g')
    ...
    link_body
      .filter(l => !l.capturing && (l.filters.bpf || l.filters.corrupt || l.filters.delay || l.filters.frequency_drop || l.filters.packet_loss))
      .append<SVGGElement>('g')
      .on('contextmenu', (datum: MapLink) => {
        const evt = event;
        this.onContextMenu.emit(new LinkContextMenu(evt, datum));
