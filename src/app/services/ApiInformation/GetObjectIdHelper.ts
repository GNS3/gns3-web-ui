...
export class GetObjectIdHelper {
  ...
  private static setLinkObjectInformation(
    links: any[],
    extraParams: IExtraParams[],
    service: ApiInformationService,
    controller: Controller
  ...
  private static getLinkInformation(
    link: any,
    extraParams: IExtraParams[],
    service: ApiInformationService,
    controller: Controller
  ): Observable<IGenericApiObject> {
    const nodesDataObs = link.nodes.map((node) =>
      service.getListByObjectId(controller, '{node_id}', node.node_id, extraParams)
    );
    return forkJoin(nodesDataObs).pipe(
      map((nodes: [any]) => {
        const name = nodes
