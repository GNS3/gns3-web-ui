...
export class PacketCaptureService {
  ...
  startCapture(controller: Controller, project: Project, link: Link, name: string) {
    const uri = `gns3+pcap://${controller.host}:${controller.port}?protocol=${controller.protocol.slice(
      0,
      -1
    )}&project_id=${project.project_id}&link_id=${link.link_id}&project=${project.name}&name=${name}`;
