...
export class ResourcePoolsService {
  constructor(private httpController: HttpController, private projectService: ProjectService) {}
  ...
  getFreeResources(controller: Controller) {
    return this.projectService.list(controller).pipe(
      switchMap((projects) => {
        return this.getAllNonFreeResources(controller).pipe(
  ...
  private getAllNonFreeResources(controller: Controller) {
    return this.getAll(controller).pipe(
