...
export class VpcsTemplatesComponent implements OnInit {
  ...
  getTemplates() {
    this.vpcsService.getTemplates(this.controller).subscribe({
      next: (vpcsTemplates: VpcsTemplate[]) => {
        this.vpcsTemplates = vpcsTemplates.filter((elem) => elem.template_type === 'vpcs' && !elem.builtin);
        this.cd.markForCheck();
      },
      error: (err) => {
        const message = err.error?.message || err.message || 'Failed to load VPCS templates';
        this.toasterService.error(message);
        this.cd.markForCheck();
      },
    });
