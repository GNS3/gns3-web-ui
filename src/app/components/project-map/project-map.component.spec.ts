...
import { MatBottomSheetModule, MatCheckboxModule, MatDialogModule, MatIconModule, MatMenuModule, MatToolbarModule } from '@angular/material';
...
import { MapDrawingsDataSource, MapLinksDataSource, MapNodesDataSource, MapSymbolsDataSource } from '../../cartography/datasources/map-datasource';
...
describe('ProjectMapComponent', () => {
  ...
  const linksDataSource = new MockedLinksDataSource();
  const mockedToasterService = new MockedToasterService();
  let nodeCreatedLabelStylesFixer;
  const mockedRouter = new MockedActivatedRoute;
  ...
  it('should hide draw tools when hide menu is called', () => {
    const dummyElement = document.createElement('map');
