...
check_file() {
  ...
  while IFS= read -r line; do
    ...
    if [ "$is_allowed_var" = false ]; then
      local colors=$(echo "$content" | grep -oE '#[0-9A-Fa-f]{3,6}' | sort -u)
      while IFS= read -r color; do
        violations="$violations$file:$line_num: $color\n"
      done <<< "$colors"
    fi
  done < <(grep -rnE '#[0-9A-Fa-f]{3,6}' "$file" 2>/dev/null)
...
if [ "$1" = "--ci" ]; then
  ...
  all_violations=""
  for file in $(find src -name "*.scss" -type f); do
  ...
  if [ -z "$STAGED_SCSS" ]; then
    exit 0
  fi

  all_violations=""
  for file in $STAGED_SCSS; do
    all_violations="$all_violations$(check_file "$file")"
  done

  if [ -n "$all_violations" ]; then
    echo "⚠️  WARNING: Hardcoded colors found in staged SCSS files"
